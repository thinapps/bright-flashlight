...
This file provides persistent instructions for AI agents and coding assistants working in the Bright Flashlight repository.
...
3. Bright Flashlight's app-specific documentation and implemented behavior
4. the shared ThinApps Android Guidelines
5. existing repository conventions not covered above

App-specific documentation is the final source of truth when Bright Flashlight intentionally differs from a shared guideline. Do not change working app behavior solely to force consistency with a general guideline when the difference is deliberate and documented.

## Product Guardrails

Preserve Bright Flashlight as a focused flashlight utility.

- Keep Torch, Strobe, SOS, Screen Light, brightness, and Auto-off behavior aligned with the app-specific documentation.
- Preserve the privacy-first baseline: no accounts, ads, analytics, tracking, or Internet permission unless the user explicitly approves and the related documentation and disclosures are updated.
- Prefer small, reliable changes over speculative features, broad refactors, or additional dependencies.
- Keep Android permissions, foreground-service behavior, failure handling, and hardware fallbacks accurately documented.
...
- update app-specific documentation or the changelog when the user-facing or release behavior requires it, using separate one-file commits
